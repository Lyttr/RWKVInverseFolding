...
PROJ_DIR="/pvc/RWKVout/L"$N_LAYER"-D"$N_EMBD"-"$MODEL_TYPE"16M0913" # set output folder
#
#######################################################################################################################
#
...
#
DS_BUCKET_MB=10 # set to 2 for consumer GPUs, set to 200 for A100 / H100 (affects speed & vram usage)
#
python train.py --load_model "0" --wandb "rna-rwkv" --proj_dir $PROJ_DIR --my_testing $MODEL_TYPE \
 ...
 --data_file "/pvc/dataset/8M0907/rna_train.npy" --my_exit_tokens 40960000000 --magic_prime 39999983 \
 ...
 --accelerator gpu --devices $GPU_PER_NODE --precision bf16 --strategy deepspeed_stage_2 --grad_cp $GRAD_CP --enable_progress_bar True --ds_bucket_mb $DS_BUCKET_MB --my_qa_mask 1
