...
PROJ_DIR="/pvc/RWKVout/L"$N_LAYER"-D"$N_EMBD"-"$MODEL_TYPE"8M0907" # set output folder
#
#######################################################################################################################
#
...
python train.py --wandb "rna-rwkv" --proj_dir $PROJ_DIR \
 --data_file "/pvc/dataset/8M0901/rna_train.npy" --data_type "numpy" --vocab_size 9 --my_testing $MODEL_TYPE \
 ...
 --accelerator cpu --devices 1 --precision fp16 --strategy deepspeed_stage_2 --grad_cp 1 --my_qa_mask 1
