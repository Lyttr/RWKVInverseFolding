...
PROJ_DIR="/pvc/RWKVout/L"$N_LAYER"-D"$N_EMBD"-"$MODEL_TYPE"8M0828" # set output folder
...
#
python inference.py --load_model "rwkv-3.pth" --wandb "rna-rwkv" --proj_dir $PROJ_DIR --my_testing $MODEL_TYPE \
 --ctx_len $CTX_LEN --my_pile_stage 3 --epoch_count 999999 --epoch_begin 0 \
 --test_json "/pvc/dataset/8M0901/rna_test_v2.json"  --output_json "/home/jovyan/RWKVInverseFolding/etherna100.json" --my_exit_tokens 114800000 --magic_prime 1793731 \
 ...
 --accelerator gpu --devices $GPU_PER_NODE --precision bf16 --strategy deepspeed_stage_2 --grad_cp $GRAD_CP --enable_progress_bar True --ds_bucket_mb $DS_BUCKET_MB --topk=1 --temperature 0.00001 --gc_target 0.25 --gc_strength 50 --div_test 0