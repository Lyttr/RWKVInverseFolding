...
class RWKV_Tmix_x052(MyModule):
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            self.time_decay.data = self.time_decay.data.to(torch.bfloat16)
            self.time_faaaa.data = self.time_faaaa.data.to(torch.bfloat16)
        ...
        def forward(self, x, x_emb=None):
            ...
            if self.args.dropout == 0:
                if self.layer_id == 0 and args.pre_ffn > 0:
                    x = x + self.ffnPre(self.ln1(x).to(x.dtype))
                else:
                    x = x + self.att(self.ln1(x).to(x.dtype))
                x = x + self.ffn(self.ln2(x).to(x.dtype))
            else:
                if self.layer_id == 0 and args.pre_ffn > 0:
...
class RWKV(pl.LightningModule):
    ...
    def forward(self, idx):
        ...
        x = self.emb(idx).to(dtype=torch.bfloat16)
